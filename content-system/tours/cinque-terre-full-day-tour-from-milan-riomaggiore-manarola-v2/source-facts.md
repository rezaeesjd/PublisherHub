# Source Facts — Cinque Terre Full-Day Tour from Milan (Variant v2)

## Variant context
- This is content variant **v2** of the canonical tour `Cinque Terre Full-Day Tour from Milan` (base package: `cinque-terre-full-day-tour-from-milan-riomaggiore-manarola`).
- Created per the multi-variant rule in `AGENTS.md`: when an existing finalized package matches the canonical tour title, generate a new package folder with a unique `slug` and `public_slug` rather than overwriting prior approved content.
- Variant angle: BOFU day-trip / "five villages" keyword variant (primary keyword `cinque terre day trip from milan`).
- Source intake is identical to the base package; all factual rows below match the base package's provenance and remain traceable to the same single supplier intake row.
...
Long description names all five Cinque Terre villages — Monterosso al Mare, Vernazza, Corniglia, Manarola, Riomaggiore. Per AGENTS.md non-blocking rule (itinerary scope conflict → pick broader scope), public copy markets the **five-village** experience.
...
## Variant differentiation notes
- Base variant page title leads with the supplier-style title: "Cinque Terre Full-Day Tour from Milan — All Five Villages by Coach, Train & Ferry".
- This v2 variant retargets the same product around the head keyword **"cinque terre day trip from milan"** for SERP coverage, with a different page title, hook, scannable structure, and FAQ angle. No factual claims diverge from the base.
