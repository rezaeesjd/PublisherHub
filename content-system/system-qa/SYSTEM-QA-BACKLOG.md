...
- 2026-05-15 | best-day-trips-from-milan | WPS:GENERATE_CONTENT | note: TOFU destination guide for cinque-terre-from-milan cluster; overview / framing facts inferred from general destination knowledge; sibling cluster cross-links to Lake Como are conditional pending live slugs; human review of overview framing recommended before publish.
- 2026-05-14 | bernina-red-train-and-st-moritz-from-milan-v2 / -v3 / -v4 | WPS:IMPROVE_SYSTEM_WORKFLOW | removed: the `-v<N>` variant mechanism was retired (it produced thin, near-duplicate clones of the source package). The three Bernina variant packages were deleted, dropped from the cluster registry, and their public URLs now 404. New content is generated as distinct typed cluster blog assets — see the "Multi-content cluster rule" in AGENTS.md. Historical entries above (lines for -v2/-v3/-v4) refer to those now-removed packages.
- 2026-05-15 | swiss-alps-from-milan-guide | WPS:GENERATE_CONTENT | action: run human review of inferred TOFU framing, then publish via WPS:PUBLISH_BLOG.
