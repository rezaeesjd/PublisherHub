...
- **Should do (automatic system QA):** Run an internal `WPS:PROCESS_QA` pass automatically at the end of generation, without requiring a separate user prompt/command, and write a per-run process report under `content-system/system-qa/reports/`.
